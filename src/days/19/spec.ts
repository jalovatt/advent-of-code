...
describe(`Day ${__filename.match(/\/([^/]+)\/spec/)?.[1]} - ${title}`, () => {
  ...
  describe('Part 1', () => {
    describe('Tests', () => {
      test('example (beacon count)', () => {
        const received = a(example1);

        if (received.length !== 79) {
          const extra = subtraction(received, split(expected1))
          ...
          log(`Got ${extra.length} extra beacons compared to the example:`);
          dir(extra);
        }
        expect(received.length).toEqual(79);
      });
    });

    describe('Solution', () => {
      const knownSolution = 303;

      test(`${knownSolution}`, () => {
        const solution = a(input);

        expect(solution.length).toEqual(knownSolution);
      });
    });
  });
  ...
  describe('Part 2', () => {
    describe('Tests', () => {
      test('example', () => {
        expect(b(example1)).toEqual(3621);
      });
    });

    describe('Solution', () => {
      const knownSolution = 9621;
